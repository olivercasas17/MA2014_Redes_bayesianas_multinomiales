...
-   Propósito del viaje
-   Tiempo de viaje
-   Clasificación del día de viaje
...
Para obtener el tiempo de cada viaje primero vamos a unir los valores de horas y minutos, tanto de salida como de llegada y luego los pasamos a formato de tiempo y restamos la llegada menos la salida.

```{r}
library(dplyr)

# Selecionar las columnas de interes para el tiempo
...
### Sexo
Para la variable de sexo solamente pasaremos el valor para que sea texto en vez de números.
```{r}
library(dplyr)
...
Para calcular el número de viajes que se hicieron por hogar, se van a sumar los viajes que se hicieron por hogar entre semana más los viajes que se hicieron por hogar los sábados.
```{r}
...
El tamaño de la localidad se separa en dos, pequeña y grande.
```{r}
library(dplyr)
...
Creamos un dataframe, definimos el nombre que tendran nuestras columnas y también definimos de que dataframe vamos a agarrar cada dato. Este dataframe solo es para las variables que se obtuvieron del archivo tviaje.csv.
```{r}
...
Ahora hacemos otro dataframe pero con las variables que se obtuvieron del archivo tsdem.csv
```{r}
df_demo = data.frame(
  "id" = df_tsdem$id_soc,
...
Y ahora los unimos, asegurandonos de que los id coincidan.
```{r}
library(dplyr)
...
Verificamos qye no existan valores nulos, y si hay los eliminamos.
```{r}
sum(is.na(data))
```
...
sum(is.na(data))
```
T por ultimo renombramos los nombres de las columnas para facilidad de lectura de la tabla y guardamos el archivo data.csv.
```{r}
